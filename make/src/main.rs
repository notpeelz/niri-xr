...
fn build<S: AsRef<str>>(args: &[S]) -> Result<BuildArtifacts> {
    ...
    let package_id = metadata.get_package_id("niri-xr")?;
    ...
    let mut build_out_dir: Option<PathBuf> = None;
    let mut niri_xr_exe: Option<PathBuf> = None;
    ...
    for line in reader.lines() {
        ...
        match msg {
            Message::BuildScriptExecuted(msg) if msg.package_id == package_id => {
                build_out_dir = Some(PathBuf::from(msg.out_dir));
            }
            Message::CompilerArtifact(artifact) if artifact.target.name == "niri-xr" => {
                if let Some(exe) = artifact.executable {
                    niri_xr_exe = Some(PathBuf::from(exe));
                }
            }
            _ => {}
        }
    ...
    let build_out_dir =
        build_out_dir.ok_or_else(|| eyre!("couldn't find niri-xr build script output dir"))?;
    let exe = niri_xr_exe.ok_or_else(|| eyre!("couldn't find niri-xr binary"))?;

    Ok(BuildArtifacts { build_out_dir, exe })
}
...
fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Command::Install {
            ...
            install_file(artifacts.exe, &dest, bin_dir.join("niri-xr"), 0o755)?;

            let systemd_user_unit_dir = lib_dir.join("systemd/user");
            install_dir(&dest, &systemd_user_unit_dir, 0o755)?;
            install_file(
                configure_file(
                    project_root.join("res/niri-xr.service.in"),
                    tmp_dir.join("niri-xr.service"),
                    &vars,
                )?,
                &dest,
                systemd_user_unit_dir.join("niri-xr.service"),
                0o644,
            )?;
            ...
            install_file(
                artifacts.build_out_dir.join("niri-xr.bash"),
                &dest,
                bash_comp_dir.join("niri-xr"),
                0o644,
            )?;
            ...
            install_file(
                artifacts.build_out_dir.join("_niri-xr"),
                &dest,
                zsh_comp_dir.join("_niri_xr"),
                0o644,
            )?;
            ...
            install_file(
                artifacts.build_out_dir.join("niri-xr.fish"),
                &dest,
                fish_comp_dir.join("niri-xr.fish"),
                0o644,
            )?;
